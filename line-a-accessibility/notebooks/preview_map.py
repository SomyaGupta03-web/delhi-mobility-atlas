...
wards = gpd.read_file(access_path if access_path.exists() else "data/interim/delhi_wards_utm43n.gpkg").to_crs(4326)
has_access = "tmin_market" in wards.columns

m = folium.Map(location=[28.61, 77.21], zoom_start=11, tiles="OpenStreetMap")

# ---- Choropleth: ward-level walk time to nearest market ----
if has_access:
    folium.Choropleth(
        geo_data=wards,
        data=wards,
...
tooltip_fields = [c for c in ["ward_id", "ward_name", "tmin_health", "tmin_education", "tmin_market"] if c in wards.columns]
tooltip_aliases = {
...
# ---- Highlight the 10 worst / 10 best wards for market access, labeled ----
if has_access:
    ranked = wards.dropna(subset=["tmin_market"]).sort_values("tmin_market")
...
<div style="position: fixed; top: 10px; left: 50px; z-index: 9999;
  ...
  <div style="font-size:16px; font-weight:bold;">Delhi Mobility Desert Index — Market Access</div>
  <div style="font-size:11px; color:#555;">Walk time is computed over the real street/footpath network (OpenStreetMap), not straight-line distance</div>
</div>
"""
...
<div style="position: fixed; bottom: 30px; left: 10px; z-index: 9999;
  ...
  <div style="margin-top:4px; color:#555;">Ward shading = walk minutes to nearest market (see color scale on map)</div>
</div>
"""
