...
<style>
  ...
  .hero-sub {{ font-family: Georgia, serif; font-style: italic; font-size: 17px; color: {RUST}; margin: 0 0 18px; }}
  ...
  .finding-box {{
    border-left: 3px solid {RUST}; background: {PAPER_ALT}; padding: 14px 18px;
...
st.markdown('<div class="hero-title">Delhi Mobility Desert Index</div>', unsafe_allow_html=True)
st.markdown('<div class="hero-sub">Where in Delhi is it genuinely hard to reach a hospital, school, or market on foot?</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown(
        "**Method:** walk time is computed via shortest-path routing over the real "
        "OpenStreetMap street network (not straight-line distance), combined with "
        "population density into a single 0-100 score."
    )
...
wards["fill_color"] = [[int(r), int(g), int(b), 195] for r, g, b in colors]
...
with col_map:
    ...
    <div class="legend-box">
      <b>{METRICS[metric][0]}</b> — teal = better, rust = worse &nbsp;|&nbsp;
...
st.markdown('<div class="section-label">Full comparison</div>', unsafe_allow_html=True)
...
st.markdown(
    ...
    <b>Finding:</b> the worst-scoring wards — Dharampura, Khajoori Khas, Jiwanpur, Gokalpur, Saboli,
    Gandhi Nagar, Mustafabad, Bhajanpura, Harsh Vihar, Karawal Nagar West — cluster almost entirely in
    <b>Northeast Delhi</b>, a part of the city independently known to be underserved. Market access is the
    weakest category citywide: roughly 31% of the walkable street network has no market within a 30-minute walk.
    </div>
    """,
    unsafe_allow_html=True,
)
